import { registerBlockType } from '@wordpress/blocks';
import { useBlockProps, InspectorControls, RichText } from '@wordpress/block-editor';
import { PanelBody, SelectControl, TextControl, Button, Card, CardBody } from '@wordpress/components';
import { Fragment } from '@wordpress/element';
import './editor.scss';
import './style.scss';

// Medical icons from Health Icons library
const MEDICAL_ICONS = {
	stethoscope: {
		name: 'Stethoscope',
		category: 'Equipment',
		path: '<path d="M8.5 1a2 2 0 1 0 0 4h.5v.5a6.5 6.5 0 0 0 13 0V5h.5a2 2 0 1 0 0-4h-14zM10 5V3h12v2a5 5 0 0 1-10 0z"/>',
	},
	heart: {
		name: 'Heart',
		category: 'Body',
		path: '<path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>',
	},
	thermometer: {
		name: 'Thermometer',
		category: 'Equipment',
		path: '<path d="M14 14.76V3.5a2.5 2.5 0 0 0-5 0v11.26a4.5 4.5 0 1 0 5 0zM12 4a1 1 0 0 1 1 1v9.17a3 3 0 1 1-2 0V5a1 1 0 0 1 1-1z"/>',
	},
	pill: {
		name: 'Pill',
		category: 'Medicine',
		path: '<path d="M8.5 1a7.5 7.5 0 0 0 0 15h7a7.5 7.5 0 0 0 0-15h-7zm0 2h7a5.5 5.5 0 0 1 0 11h-7a5.5 5.5 0 0 1 0-11z"/>',
	},
	syringe: {
		name: 'Syringe',
		category: 'Equipment',
		path: '<path d="M2.5 7.5L7 12l1.5-1.5 7.5 7.5 3-3-7.5-7.5L12 7l4.5-4.5-3-3L9 4l.5 1.5L7 8l1.5 1.5z"/>',
	},
	'first-aid': {
		name: 'First Aid Cross',
		category: 'Emergency',
		path: '<path d="M3 3h18v18H3V3zm2 2v14h14V5H5zm6 2h2v4h4v2h-4v4h-2v-4H7v-2h4V7z"/>',
	},
	ambulance: {
		name: 'Ambulance',
		category: 'Emergency',
		path: '<path d="M2 8h4l2-2h8l2 2h4v8h-2v2h-2v-2H6v2H4v-2H2V8zm2 2v4h2v-2h2v2h8v-2h2v2h2v-4h-3l-1-1H7l-1 1H3z"/>',
	},
	tooth: {
		name: 'Tooth',
		category: 'Dental',
		path: '<path d="M12 2a6 6 0 0 0-6 6c0 1.5.5 3 1 4.5L8 16c.5 2 1.5 4 2 6h4c.5-2 1.5-4 2-6l1-3.5c.5-1.5 1-3 1-4.5a6 6 0 0 0-6-6z"/>',
	},
	bandage: {
		name: 'Bandage',
		category: 'Treatment',
		path: '<path d="M3 8h2l2-2 8 8-2 2h-2l-8-8zm1 1l7 7h1l1-1L6 8H4zm8-6l3 3-1 1-3-3 1-1zm4 4l3 3-1 1-3-3 1-1z"/>',
	},
	'x-ray': {
		name: 'X-Ray',
		category: 'Imaging',
		path: '<path d="M4 4h16v16H4V4zm2 2v12h12V6H6zm2 2h8v8H8V8zm2 2v4h4v-4h-4z"/>',
	},
	calendar: {
		name: 'Calendar',
		category: 'Schedule',
		path: '<path d="M3 4h18v16H3V4zm2 2v12h14V6H5zm2-4v2h2V2H7zm6 0v2h2V2h-2z"/>',
	},
	shield: {
		name: 'Shield',
		category: 'Protection',
		path: '<path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4z"/>',
	},
};

const ICON_OPTIONS = Object.entries(MEDICAL_ICONS).map(([key, icon]) => ({
	label: icon.name,
	value: key,
}));

registerBlockType('global360blocks/info-cards', {
	edit: (props) => {
		const { attributes, setAttributes } = props;
		const { mainTitle, cards } = attributes;

		const blockProps = useBlockProps({
			className: 'info-cards-block',
		});

		const renderIcon = (iconKey, size = 48) => {
			const icon = MEDICAL_ICONS[iconKey];
			if (!icon) return null;

			return (
				<svg
					width={size}
					height={size}
					viewBox="0 0 24 24"
					fill="var(--cpt360-primary, #0073aa)"
					stroke="var(--cpt360-primary, #0073aa)"
					strokeWidth="2"
					strokeLinecap="round"
					strokeLinejoin="round"
					className="info-card-icon"
				>
					<g dangerouslySetInnerHTML={{ __html: icon.path }} />
				</svg>
			);
		};

		const updateCard = (cardIndex, field, value) => {
			const newCards = [...cards];
			newCards[cardIndex] = { ...newCards[cardIndex], [field]: value };
			setAttributes({ cards: newCards });
		};

		const addCard = () => {
			const newCards = [
				...cards,
				{
					icon: 'stethoscope',
					title: 'New Card',
					text: 'Add your content here.',
				},
			];
			setAttributes({ cards: newCards });
		};

		const removeCard = (cardIndex) => {
			const newCards = cards.filter((_, index) => index !== cardIndex);
			setAttributes({ cards: newCards });
		};

		return (
			<Fragment>
				<InspectorControls>
					<PanelBody
						title="Main Settings"
						initialOpen={true}
					>
						<TextControl
							label="Main Title"
							value={mainTitle}
							onChange={(value) => setAttributes({ mainTitle: value })}
						/>
					</PanelBody>

					<PanelBody
						title="Cards"
						initialOpen={true}
					>
						{cards.map((card, index) => (
							<Card
								key={index}
								className="card-settings"
							>
								<CardBody>
									<h4>Card {index + 1}</h4>

									<SelectControl
										label="Icon"
										value={card.icon}
										options={ICON_OPTIONS}
										onChange={(value) => updateCard(index, 'icon', value)}
									/>

									<TextControl
										label="Title"
										value={card.title}
										onChange={(value) => updateCard(index, 'title', value)}
									/>

									<TextControl
										label="Text"
										value={card.text}
										onChange={(value) => updateCard(index, 'text', value)}
										help="Brief description for this card"
									/>

									{cards.length > 1 && (
										<Button
											isDestructive
											variant="secondary"
											onClick={() => removeCard(index)}
											className="remove-card-btn"
										>
											Remove Card
										</Button>
									)}
								</CardBody>
							</Card>
						))}

						{cards.length < 6 && (
							<Button
								variant="primary"
								onClick={addCard}
								className="add-card-btn"
							>
								Add Card
							</Button>
						)}
					</PanelBody>
				</InspectorControls>

				<div {...blockProps}>
					<div className="info-cards-container">
						<RichText
							tagName="h2"
							className="info-cards-main-title"
							value={mainTitle}
							onChange={(value) => setAttributes({ mainTitle: value })}
							placeholder="Enter main title..."
						/>

						<div className="info-cards-grid">
							{cards.map((card, index) => (
								<div
									key={index}
									className="info-card"
								>
									<div className="info-card-icon-wrapper">{renderIcon(card.icon, 64)}</div>
									<RichText
										tagName="h3"
										className="info-card-title"
										value={card.title}
										onChange={(value) => updateCard(index, 'title', value)}
										placeholder="Enter card title..."
									/>
									<RichText
										tagName="p"
										className="info-card-text"
										value={card.text}
										onChange={(value) => updateCard(index, 'text', value)}
										placeholder="Enter card description..."
									/>
								</div>
							))}
						</div>
					</div>
				</div>
			</Fragment>
		);
	},

	save: () => {
		// Rendered by PHP
		return null;
	},
});
